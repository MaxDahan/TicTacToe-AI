...
public class TTT implements KeyListener {
	...
	//start
	public TTT() {
		for(int i = 0; i < board.length; i++) {board[i] = " ";} //so board is filled with at least something
		...
		start.setLocation(1920/2 - start.getWidth()/2, 1080/2 - start.getHeight()/2);
	...
	private class ezz implements ActionListener, KeyListener {
	...
	private class med implements ActionListener, KeyListener {
		public void actionPerformed(ActionEvent arg0) {
			difficulty = "medium";
			int x = dif.getX();
			int y = dif.getY();
	...
	private class hard implements ActionListener, KeyListener {
		public void actionPerformed(ActionEvent arg0) {
			difficulty = "hard";
			int x = dif.getX();
			int y = dif.getY();
	...
	private class check extends Thread implements KeyListener {
		public void run() {
			while(play) {
	...
	private class restart implements ActionListener, KeyListener {
		public void actionPerformed(ActionEvent arg0) {
			int x = TTF.getX();
			int y = TTF.getY();
			TTF.dispose();
	...
	//Closes current frame when called
	public void closeMe() {
		if(frame.equals("start")) {
			start.dispose();
		} else if(frame.equals("game")) {
	...
	//AI button
	private class AI implements ActionListener, KeyListener {
	...
	//Friend button
	private class friend implements ActionListener, KeyListener {
		public void actionPerformed (ActionEvent e) {
			int x = start.getX();
			int y = start.getY();
			TTF = new JFrame("Tic-Tac-Toe");
